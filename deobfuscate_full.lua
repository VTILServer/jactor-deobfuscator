--[[============================================================================
  deobfuscate_full.lua  -  self-detecting devirtualizer that outputs REAL Lua.

  Same auto-detection front-end as deobfuscate_auto.lua (opcodes and field
  slots are re-derived from each sample's own VM, so opcode shuffling is
  handled automatically).  But instead of a pseudocode listing, this script
  rebuilds genuine Lua 5.1 *bytecode* from the devirtualized instructions
  (this obfuscator is a 1:1 virtualization of Lua 5.1 bytecode) and then runs
  unluac.jar to decompile it back into real, structured Lua source.

  Usage:
      lua deobfuscate_full.lua <input.lua> [output.lua]

  Requirements: a Java runtime and unluac.jar (shipped in this repo).  If they
  are missing the script still writes the reconstructed <output>.luac bytecode
  and tells you the unluac command to run.
...
local src_path = assert(arg and arg[1], 'usage: lua deobfuscate_full.lua <input.lua> [output.lua]')
local out_path = arg and arg[2] or (src_path:gsub('%.lua$', '') .. '.decompiled.lua')
...
local function decode_proto(raw, det, layout, d1, d2, d3)
	local node = {
		params    = tonumber(raw[layout.params]) or 0,
		nups      = tonumber(raw[2]) or 0,   -- slot [2] holds the upvalue count
...
--=============================================================================
-- 5. Lua 5.1 bytecode emitter
--    Each VM instruction maps 1:1 onto a Lua 5.1 bytecode instruction, so we
--    rebuild a standard luac chunk and hand it to unluac for real source.
--=============================================================================

-- canonical operation name -> Lua 5.1 opcode number
local LUA_OP = {
...
-- byte packing helpers (little-endian).  Match the running platform's Lua 5.1
-- header sizes so the .luac is also natively loadable (unluac reads the header
-- regardless).  int=byte8, size_t=byte9, Instruction=byte10, Number=byte11.
local REF_HEADER = string.dump(function() end)
local SIZE_T = REF_HEADER:byte(9)        -- 4 on 32-bit, 8 on 64-bit builds
local function u8(n) return string.char(math.floor(n) % 256) end
local function u32(n)
	n = math.floor(n) % 4294967296
	return string.char(n % 256, math.floor(n / 256) % 256,
		math.floor(n / 65536) % 256, math.floor(n / 16777216) % 256)
end
local function sizet(n)
	if SIZE_T == 4 then return u32(n) end
	return u32(n) .. u32(math.floor(n / 4294967296))
end
local function dump_string(s)
	if s == nil then return sizet(0) end
	return sizet(#s + 1) .. s .. '\0'
end
local function dump_number(x)
	x = x + 0.0
	if x ~= x then return string.char(0,0,0,0,0,0,0xF8,0x7F) end       -- nan
	if x == math.huge then return string.char(0,0,0,0,0,0,0xF0,0x7F) end
	if x == -math.huge then return string.char(0,0,0,0,0,0,0xF0,0xFF) end
	if x == 0 then
		if 1 / x == -math.huge then return string.char(0,0,0,0,0,0,0,0x80) end
	...
	local exp = e - 1 + 1023
	local mant = (m * 2 - 1) * 2 ^ 52
	mant = math.floor(mant + 0.5)
	if mant >= 2 ^ 52 then mant = mant - 2 ^ 52; exp = exp + 1 end
	local mant_low  = mant % 4294967296
	local mant_high = math.floor(mant / 4294967296)          -- 20 bits
	local b1 = mant_low % 256
	local b2 = math.floor(mant_low / 256) % 256
	local b3 = math.floor(mant_low / 65536) % 256
	local b4 = math.floor(mant_low / 16777216) % 256
	local b5 = mant_high % 256
	local b6 = math.floor(mant_high / 256) % 256
	local b7 = (math.floor(mant_high / 65536) % 16) + (exp % 16) * 16
	local b8 = (math.floor(exp / 16) % 128) + sign * 128
	return string.char(b1, b2, b3, b4, b5, b6, b7, b8)
end

local BITRK = 256                                            -- RK constant flag
local MAXARG_sBx = 131071

local emit_warnings = {}
local function ewarn(msg) emit_warnings[#emit_warnings + 1] = msg end

-- assemble one proto: fill proto._code (encoded ints), proto._consts, _maxstack
local function assemble(proto, path)
	local consts, cindex = {}, {}
	local function K(v)
		local key = type(v) .. '\1' .. tostring(v)
		if cindex[key] ~= nil then return cindex[key] end
		consts[#consts + 1] = v
		cindex[key] = #consts - 1
		return #consts - 1
	end
	local maxreg = 1
	local function useR(n) if n and n < 250 and n > maxreg then maxreg = n end end

	local code = {}
	for pc, inst in ipairs(proto.instructions) do
		local name = inst.opname
		local op = LUA_OP[name]
		local A = math.floor(tonumber(inst.A) or 0)
		local B = math.floor(tonumber(inst.B) or 0)
		local C = math.floor(tonumber(inst.C) or 0)
		local function rkB() if inst.isKB then return K(inst.KB) + BITRK else useR(B); return B end end
		local function rkC() if inst.isKC then return K(inst.KC) + BITRK else useR(C); return C end end
		useR(A)
		local word
		if op == nil then
			ewarn(string.format('%s pc %d: unknown op %s, emitted as RETURN', path, pc, tostring(name)))
			word = LUA_OP.RETURN + A * 64 + 1 * 16384        -- return (B=1)
		elseif name == 'JMP' or name == 'FORLOOP' or name == 'FORPREP' then
			local sBx = math.floor(tonumber(inst.sBx) or 0)
			word = op + A * 64 + (sBx + MAXARG_sBx) * 16384
		elseif name == 'LOADK' or name == 'GETGLOBAL' or name == 'SETGLOBAL' then
			word = op + A * 64 + K(inst.Kst) * 16384
		elseif name == 'CLOSURE' then
			word = op + A * 64 + (math.floor(tonumber(inst.proto) or 0)) * 16384
		else
			-- iABC with per-op RK handling
			local b, c = B, C
			if name == 'ADD' or name == 'SUB' or name == 'MUL' or name == 'DIV'
				or name == 'MOD' or name == 'POW' or name == 'EQ' or name == 'LT'
				or name == 'LE' or name == 'SETTABLE' then
				b, c = rkB(), rkC()
			...
			if name == 'SETLIST' and C == 0 then
				ewarn(string.format('%s pc %d: SETLIST with C=0 (extended count) not fully supported', path, pc))
			end
			word = op + A * 64 + c * 16384 + b * 8388608
		end
		code[pc] = u32(word)
	end
	...
	proto._maxstack = math.max(2, math.min(250, maxreg + 3))

	local is_vararg = 0
	for _, inst in ipairs(proto.instructions) do
		if inst.opname == 'VARARG' then is_vararg = 2 end
	end
	proto._is_vararg = is_vararg

	for _, sub in ipairs(proto.subprotos) do assemble(sub, path .. '.sub') end
end

local function dump_constant(v)
	local t = type(v)
	if v == nil then return u8(0)
	elseif t == 'boolean' then return u8(1) .. u8(v and 1 or 0)
	elseif t == 'number' then return u8(3) .. dump_number(v)
	elseif t == 'string' then return u8(4) .. dump_string(v)
	else ewarn('unsupported constant type ' .. t); return u8(0) end
end

local function dump_proto(proto, is_main)
	local parts = {}
	parts[#parts + 1] = dump_string(is_main and '@deobfuscated' or nil)  -- source
	parts[#parts + 1] = u32(0)                     -- line defined
	parts[#parts + 1] = u32(0)                     -- last line defined
	parts[#parts + 1] = u8(proto.nups or 0)        -- nups
	parts[#parts + 1] = u8(proto.params or 0)      -- numparams
	parts[#parts + 1] = u8(is_main and 2 or proto._is_vararg)  -- is_vararg
	parts[#parts + 1] = u8(proto._maxstack)        -- maxstacksize
	-- code
	parts[#parts + 1] = u32(#proto._code)
	for _, w in ipairs(proto._code) do parts[#parts + 1] = w end
	-- constants
	parts[#parts + 1] = u32(#proto._consts)
	for _, c in ipairs(proto._consts) do parts[#parts + 1] = dump_constant(c) end
	-- nested prototypes
	parts[#parts + 1] = u32(#proto.subprotos)
	for _, sub in ipairs(proto.subprotos) do parts[#parts + 1] = dump_proto(sub, false) end
	-- debug (stripped): lineinfo, locvars, upvalues
	parts[#parts + 1] = u32(0)
	parts[#parts + 1] = u32(0)
	parts[#parts + 1] = u32(0)
...
local function dump_chunk(main)
	local header = '\27Lua'
		.. u8(0x51)  -- version 5.1
		.. u8(0)     -- official format
		.. u8(1)     -- little endian
		.. u8(4)     -- sizeof(int)
		.. u8(SIZE_T)  -- sizeof(size_t) (matches this platform)
		.. u8(4)     -- sizeof(Instruction)
		.. u8(8)     -- sizeof(lua_Number)
		.. u8(0)     -- number is floating point
...
-- attach op names to every instruction, recursively
local function name_instructions(proto)
	for _, inst in ipairs(proto.instructions) do
		inst.opname = det.opmap[inst.op] or ('OP_' .. tostring(inst.op))
	end
	for _, sub in ipairs(proto.subprotos) do name_instructions(sub) end
end
...
name_instructions(proto)

assemble(proto, 'main')
local bytecode = dump_chunk(proto)

local luac_path = out_path:gsub('%.lua$', '') .. '.luac'
write_all(luac_path, bytecode)

-- console + detection summary
io.write('[deobfuscate_full] detected ' .. det.leaf_count .. ' handlers; ' ..
	#det.warnings .. ' detection warning(s), ' .. #emit_warnings .. ' emit warning(s)\n')
for _, w in ipairs(det.warnings) do io.write('  DETECT WARNING: ' .. w .. '\n') end
for _, w in ipairs(emit_warnings) do io.write('  EMIT WARNING: ' .. w .. '\n') end
io.write('[deobfuscate_full] wrote reconstructed bytecode -> ' .. luac_path .. '\n')

-- try to locate unluac.jar (cwd or next to this script) and run it
local function file_exists(p) local f = io.open(p, 'rb') if f then f:close() return true end return false end
local jar
for _, cand in ipairs{ 'unluac.jar', (src_path:match('^(.*[\\/])') or '') .. 'unluac.jar' } do
	if file_exists(cand) then jar = cand; break end
end

if not jar then
	io.write('[deobfuscate_full] unluac.jar not found; decompile manually:\n')
	io.write('    java -jar unluac.jar "' .. luac_path .. '" > "' .. out_path .. '"\n')
	return
end

-- build a header describing the detected opcode map, prepended to the source
local nums = {}
for k in pairs(det.opmap) do nums[#nums + 1] = k end
table.sort(nums)
local map_line = ''
for _, n in ipairs(nums) do map_line = map_line .. n .. '=' .. det.opmap[n] .. ' ' end

local cmd = 'java -jar "' .. jar .. '" "' .. luac_path .. '"'
local pipe = io.popen(cmd, 'r')
local decompiled = pipe and pipe:read('*a') or nil
local ok = pipe and pipe:close()

if not decompiled or decompiled == '' then
	io.write('[deobfuscate_full] unluac produced no output; run manually:\n')
	io.write('    ' .. cmd .. ' > "' .. out_path .. '"\n')
	return
end

local header = table.concat({
	'-- Decompiled with deobfuscate_full.lua (auto-detected VM) + unluac',
	'-- Source: ' .. src_path,
	'-- Detected opcode map: ' .. map_line,
	'',
	'',
}, '\n')
write_all(out_path, header .. decompiled)
io.write('[deobfuscate_full] wrote real Lua source -> ' .. out_path .. '\n')
